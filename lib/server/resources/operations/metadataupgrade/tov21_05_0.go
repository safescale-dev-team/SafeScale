 ...
 func (tv toV21_05_0) upgradeNetworkMetadataIfNeeded(instance resources.Network) fail.Error {
	 ...
	 xerr := instance.Alter(func(clonable data.Clonable, props *serialize.JSONProperties) fail.Error {
		 ...
		 if !props.Lookup(networkproperty.SubnetsV1) {
			 ...
			 gwSG, internalSG, publicSG, innerXErr := subnetInstance.(*operations.Subnet).UnsafeCreateSecurityGroups(context.Background(), instance, false, int32(abstractSubnet.DefaultSSHPort))
			 if innerXErr != nil {
				 return innerXErr
			 }
